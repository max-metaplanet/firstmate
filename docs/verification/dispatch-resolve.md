...
It proves the path-force precheck: a declared deployment-configuration path selects the rule declaring `path_force` with no `curl` call while still reading quota and ranking that rule's candidates, a home whose rules sit in a different order forces the same rule by declaration rather than by index, every path class in the fact list and its reported pattern, a declared path that is never expanded against the working directory, bulleted, backticked, or comma-separated declaration lines, near-miss paths, and prose mentions falling through to the model unchanged, captain approval and rule-floor fall-through behaving identically on a forced route, and malformed or duplicated `path_force` declarations exiting 2.
