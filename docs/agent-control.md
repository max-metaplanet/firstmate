...
The indicator and the key that returns a composer to text entry are owned by `bin/fm-composer-lib.sh`, because ordinary text delivery hits the same condition and recovers from it the same way.
