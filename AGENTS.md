...
config/claude-seat config/claude-seats-root config/claude-seat-threshold config/claude-seat-destination-min config/claude-seat-extra-usage  optional active Claude seat (account) for NEW claude workers, the root holding seat profile directories, the percent LEFT on the active seat that trips an automatic switch, the percent LEFT a seat must exceed to be a switch destination, and what to do when no seat qualifies ("stop" or "allow <usd>" of paid extra usage); every percentage counts percent left, and absent means today's single-account behaviour, no automatic switching, and no dispatch hold; LOCAL, gitignored; inherited by local secondmate homes, which a home declines for itself with the presence flag config/claude-seat-local so it can spend a separate account, but never sent to a remote route; bin/fm-seat.sh is the one command that reads and writes them, reporting every declining home in its status, and a switch never moves a running or relaunched worker because each task records its own seat; a hold stops work being STARTED and never stops a running worker drawing extra usage, so it is not a guarantee of zero spend; see docs/configuration.md "Claude seats" and docs/claude-seats.md
...
That blocked reading is the gate working, not a stuck worker, so steer the worker on what the refusal names rather than waiting: usually an unpushed commit, or for direct-PR a PR opened on a repository other than `origin`.
