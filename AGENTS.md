...
config/claude-seat config/claude-seats-root config/claude-seat-threshold  optional active Claude seat (account) for NEW claude workers, the root holding seat profile directories, and the percent-remaining that trips an automatic switch; absent means today's single-account behaviour and no automatic switching; LOCAL, gitignored; inherited by secondmate homes; bin/fm-seat.sh is the one command that reads and writes them, and a switch never moves a running or relaunched worker because each task records its own seat; see docs/configuration.md "Claude seats" and docs/claude-seats.md
