#!/usr/bin/env bash
# Live Herdr lab driver (round 2: proof reads whole capture, 200 rows).
...
case "$FLOW" in
  probe-proof)
    ...
    for payload in "/exit" "hello world"; do
      ...
      for _ in 1 2 3; do lab pane send-keys "$PANE" ctrl+u >/dev/null; done; sleep 0.8
    done ;;
  send-plain)
    . "$CODE/bin/fm-backend.sh"; fm_backend_source herdr
    T="$SESSION:$PANE"
    log "fm_backend_herdr_send_text_submit plain text (the fm-send path)"
    v=$(fm_backend_herdr_send_text_submit "$T" "Reply with only the single word PONGLAB and nothing else." 3 0.3 1.2)
    log "verdict=$v"
    case "$v" in send-failed|unknown|'') RESULT=1 ;; esac
    for _ in $(seq 1 30); do screen | grep -q '^[^❯]*PONGLAB' && break; sleep 1; done
    if screen | grep -v 'Reply with' | grep -q PONGLAB; then log "claude answered PONGLAB: submitted"; else log "no PONGLAB reply"; RESULT=1; fi
    screen | grep -v '^[[:space:]]*$' | tail -10 ;;
  plain-exit)
    run_exit ;;
  interrupt-exit)
  ...
  busy-interrupt)
    ...
    footer
    lab pane send-keys "$PANE" ctrl+u >/dev/null; sleep 0.5
    run_exit ;;
  relaunch)
    log "\$ fm-control.sh mc1 interrupt"
    OUT=$(ctl mc1 interrupt); rc=$?; printf '%s\n' "$OUT"; log "interrupt rc=$rc"
