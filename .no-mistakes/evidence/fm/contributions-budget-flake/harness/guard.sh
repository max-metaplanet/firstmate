...
. "/Users/max/.no-mistakes/worktrees/61c6dd39b831/01M396Q7Q6VS9QH0XF1QBRB74Q/tests/lib.sh"
...
home=$(new_home guard); forge_home "$home"; wrap_forge "$home"
printf 'reserve\n' > "$home/forge/fault"
for v in '' 'garbage' '1790000000'; do
  printf '%s' "$v" > "$home/forge/clock"
  FORGE="$home/forge" PATH="$home/fakebin:$PATH" "$home/fakebin/gh" api repos/o/r/issues/9 >/dev/null 2>"$home/err"; rc=$?
  echo "clock=[$v] -> gh exit=$rc stderr=[$(cat "$home/err")] clock-after=[$(cat "$home/forge/clock")] leftover-tmp=$(ls "$home/forge" | grep -c '^clock\.' )"
done
