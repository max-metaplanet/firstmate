...
case "$fault:$*" in
  ...
  reserve:'api repos/o/r/'*)
    v=$(( $(cat "$FORGE/clock") + 6 )); : > "$FORGE/clock"; sleep 0.05; printf '%s\n' "$v" > "$FORGE/clock" ;;
...
failures=0
BASE_TMP=$TMP_ROOT; for i in $(seq 1 "${REPS:-1}"); do TMP_ROOT=$BASE_TMP/rep$i; mkdir -p $TMP_ROOT;
  for t in $TESTS; do ( "$t" ) >/dev/null 2>"$TMP_ROOT/err" || { failures=$((failures+1)); echo "FAIL rep=$i $t: $(tail -1 "$TMP_ROOT/err")"; }; done
done
echo "RESULT failures=$failures runs=$(( ${REPS:-1} * $(echo $TESTS | wc -w) ))"
