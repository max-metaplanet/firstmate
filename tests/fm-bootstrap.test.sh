...
declared path_force is accepted^{"rules":[{"when":"deployment configuration","path_force":"deployment-config","use":{"harness":"claude","model":"opus"}}]}^empty^
unknown path_force is flagged^{"rules":[{"when":"deployment configuration","path_force":"deploy","use":{"harness":"claude"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - path_force must be "deployment-config" when present
two path_force rules are flagged^{"rules":[{"when":"a","path_force":"deployment-config","use":{"harness":"claude"}},{"when":"b","path_force":"deployment-config","use":{"harness":"codex"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - at most one rule may declare path_force
